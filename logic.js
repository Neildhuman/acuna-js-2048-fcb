...
let is2048Exists = false;
let is4096Exists = false;
let is8192Exists = false;
...
function handleSlide(e){
	...
	if(["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.code)){
		// Prevents default behavior (scrolling) on keydown
		e.preventDefault();
...
function slideDown(){
	for(let c = 0; c < columns; c++){
		...
		row.reverse();
		row = slide(row);
		row.reverse(); // One done in merging, we set it back to it's original sequence through reversing again
...
function checkWin(){
	for(let r = 0; r < rows; r++){
		for(let c = 0; c < columns; c++){
			if(board[r][c] == 2048 && is2048Exists == false){
				alert("You win! You got the 2048");
				is2048Exists = true;
			} else if(board[r][c] == 4096 && is4096Exists == false){
				alert("You're unstoppable at 4096. Good job!");
				is4096Exists = true;
			} else if(board[r][c] == 8192 && is8192Exists == false){
				alert("Victory for reaching 8192. Incredible work!");
				is8192Exists = true;
			}
		}
	}
